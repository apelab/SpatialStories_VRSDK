...
/// but before that One game object has to satisfy its gaze condition OR the other one its proximity condition
/// not before.
///
/// Tests Passes On:
/// If the trigger is fired when the object has one of its dependencies satisfied.
/// 
/// Tests Fails On:
/// After a timeout or if he triggers when all has been validated because now it is only required one dependent condition to 
/// trigger.
/// </summary>
public class Dependencies_AfterProximityOrGaze : Gaze_SucceedOnTriggerTest
{
