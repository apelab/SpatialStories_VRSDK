...
Shader "Special FX/FX Dissolve Distortion" {
Properties {
	...
	SubShader {
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				{
					c.rgb = i.color.rgb * _TintColor.rgb * c.rgb;
				...
				{
					c.rgb = 4.0f * i.color.rgb * _TintColor.rgb * c.rgb;
					c.a = 2.0f * i.color.a * _TintColor.a * c.a;
					UNITY_APPLY_FOG_COLOR(i.fogCoord, c, fixed4(0.5,0.5,0.5,0.5));
				}
				if (_Mode == 7)//Alpha Blended Premultiply
				{
					c = 2.0f * i.color * _TintColor * c * i.color.a;
				}
				...
				c.rgb = 1-(1-(c.rgb*_Con))*_Con + _Bri;//Brightness, Contrast								
				return c;
			}
			ENDCG 
		}
