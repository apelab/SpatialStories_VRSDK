...
namespace Gaze
{
    ...
    public class Gaze_ActionsEditor : Gaze_Editor
    {
        ...
        private bool expandObjectPlacementList = false;
...
//                GUILayout.EndHorizontal();
        ...
        private void ShowARPlaceOptions()
        {
            actionsScript.IsMoveObjectRequired = EditorGUILayout.Toggle("Move on Touch:", actionsScript.IsMoveObjectRequired);

            if (actionsScript.IsMoveObjectRequired)
            {
                actionsScript.instantMove = EditorGUILayout.Toggle("Move Instantly:", actionsScript.instantMove);
                if (!actionsScript.instantMove)
                    actionsScript.moveSpeed = EditorGUILayout.FloatField("Move Speed:", actionsScript.moveSpeed);
                actionsScript.dropHeight = EditorGUILayout.FloatField("Drop Heigth: ", actionsScript.dropHeight);
                EditorGUILayout.HelpBox("This action includes a 'Touch Screen' condition built in it. Therefore you do not need to set up a 'Touch' condition to us it. However the conditions still need to be validated to place objects", MessageType.Info);
            }
        }
