...
namespace Gaze
{
    ...
    public class Gaze_Actions : Gaze_AbstractBehaviour
    {
        ...
        public enum AR_PLACING_OPTIONS { NOTHING, MOVE_THIS_OBJECT }
        ...
        //AR Place
        public bool IsMoveObjectRequired;
        ...
        private void MoveWithSpeed(Vector3 _currentPosition, Vector3 _destinationPosition)
        {
            // get the lerp component
            LerpObjectPosition lerpComponent = GetIO().GetComponent<LerpObjectPosition>();

            // add a move component to the object
            if (lerpComponent == null && !instantMove)
                lerpComponent = GetIO().gameObject.AddComponent<LerpObjectPosition>();

            // start a new trip for the object
            lerpComponent.SetNewTrip(previousPosition, _destinationPosition, moveSpeed);
        }
